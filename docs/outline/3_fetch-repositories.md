...
2. Level 2: issue 一覧を取得する
3. Level 3: issue の詳細情報を取得する
...
リポジトリ一覧を取得する場合には、クエリ `repositoriesQuery` を使用します。
...
ここで、非同期で取得するための関数 `fetchRepositories()` を作成します。
